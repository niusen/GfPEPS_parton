...
AA5_Phyodd_Vodd_Vodd=Matrix(undef,l_Vodd,l_Vodd);#upper layer, lower layer
AA5_Phyodd_Vodd_Veven=Matrix(undef,l_Vodd,l_Veven);#upper layer, lower layer
AA5_Phyodd_Veven_Vodd=Matrix(undef,l_Veven,l_Vodd);#upper layer, lower layer
AA5_Phyodd_Veven_Veven=Matrix(undef,l_Veven,l_Veven);#upper layer, lower layer

AA5_Phyeven_Vodd_Vodd=Matrix(undef,l_Vodd,l_Vodd);#upper layer, lower layer
AA5_Phyeven_Vodd_Veven=Matrix(undef,l_Vodd,l_Veven);#upper layer, lower layer
AA5_Phyeven_Veven_Vodd=Matrix(undef,l_Veven,l_Vodd);#upper layer, lower layer
AA5_Phyeven_Veven_Veven=Matrix(undef,l_Veven,l_Veven);#upper layer, lower layer
...
for cc1=1:l_Vodd
    for cc2=1:l_Vodd
        ...
        @tensor Ap_temp[:]:=A5'[-1,1,-3,-4,2]*V_odd[cc1][-2,1]*Phy_odd'[2,-5];
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_odd[cc2]'[1,-2]*Phy_odd[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyodd_Vodd_Vodd[cc1,cc2]=AA_temp;

        @tensor Ap_temp[:]:=A5'[-1,1,-3,-4,2]*V_odd[cc1][-2,1]*Phy_even'[2,-5];
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_odd[cc2]'[1,-2]*Phy_even[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyeven_Vodd_Vodd[cc1,cc2]=AA_temp;
...
for cc1=1:l_Vodd
    for cc2=1:l_Veven
        ...
        @tensor Ap_temp[:]:=A5'[-1,1,-3,-4,2]*V_odd[cc1][-2,1]*Phy_odd'[2,-5];
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_even[cc2]'[1,-2]*Phy_odd[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyodd_Vodd_Veven[cc1,cc2]=AA_temp;

        @tensor Ap_temp[:]:=A5'[-1,1,-3,-4,2]*V_odd[cc1][-2,1]*Phy_even'[2,-5];
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_even[cc2]'[1,-2]*Phy_even[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyeven_Vodd_Veven[cc1,cc2]=AA_temp;
...
for cc1=1:l_Veven
    for cc2=1:l_Vodd
        ...
        @tensor Ap_temp[:]:=A5'[-1,1,-3,-4,2]*V_even[cc1][-2,1]*Phy_odd'[2,-5];
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_odd[cc2]'[1,-2]*Phy_odd[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyodd_Veven_Vodd[cc1,cc2]=AA_temp;

        @tensor Ap_temp[:]:=A5'[-1,1,-3,-4,2]*V_even[cc1][-2,1]*Phy_even'[2,-5];
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_odd[cc2]'[1,-2]*Phy_even[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyeven_Veven_Vodd[cc1,cc2]=AA_temp;
...
for cc1=1:l_Veven
    for cc2=1:l_Veven
        ...
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_even[cc2]'[1,-2]*Phy_odd[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyodd_Veven_Veven[cc1,cc2]=AA_temp;

        @tensor Ap_temp[:]:=A5'[-1,1,-3,-4,2]*V_even[cc1][-2,1]*Phy_even'[2,-5];
        @tensor A_temp[:]:=A5[-1,1,-3,-4,2]*V_even[cc2]'[1,-2]*Phy_even[-5,2];
        AA_temp, U_L,U_D,U_R,U_U=build_double_layer_NoSwap(Ap_temp,A_temp);
        AA5_Phyeven_Veven_Veven[cc1,cc2]=AA_temp;
...
global AA5_Phyeven_Vodd_Vodd, AA5_Phyeven_Vodd_Veven, AA5_Phyeven_Veven_Vodd, AA5_Phyeven_Veven_Veven
global AA5_Phyodd_Vodd_Vodd, AA5_Phyodd_Vodd_Veven, AA5_Phyodd_Veven_Vodd, AA5_Phyodd_Veven_Veven
global AA2_Phyodd, AA2_Phyeven,AA3_Phyodd, AA3_Phyeven,AA4_Phyodd, AA4_Phyeven
                ...
                for parity_P3=0:1
                    for parity_P4=0:1
                        for parity_P5=0:1
                                ...
                                if parity_P5==1
                                    AA5_temp=deepcopy(AA5_Phyodd_Vodd_Vodd[cc1,cc2]);
                                elseif parity_P5==0
                                    AA5_temp=deepcopy(AA5_Phyeven_Vodd_Vodd[cc1,cc2]);
                                end
                ...
                for parity_P3=0:1
                    for parity_P4=0:1
                        for parity_P5=0:1
                                ...
                                if parity_P5==1
                                    AA5_temp=deepcopy(AA5_Phyodd_Vodd_Veven[cc1,cc2]);
                                elseif parity_P5==0
                                    AA5_temp=deepcopy(AA5_Phyeven_Vodd_Veven[cc1,cc2]);
                                end
                                gate=parity_gate(AA1_temp,4); @tensor AA1_temp[:]:=AA1_temp[-1,-2,-3,1]*gate[-4,1];#sign of U1,U1'
                                if parity_P2==1
                                    gate=parity_gate(AA1_temp,3); @tensor AA1_temp[:]:=AA1_temp[-1,-2,1,-4]*gate[-3,1];#sign P2*(R1+R1')
                ...
                for parity_P3=0:1
                    for parity_P4=0:1
                        for parity_P5=0:1
                                ...
                                if parity_P5==1
                                    AA5_temp=deepcopy(AA5_Phyodd_Veven_Vodd[cc1,cc2]);
                                elseif parity_P5==0
                                    AA5_temp=deepcopy(AA5_Phyeven_Veven_Vodd[cc1,cc2]);
                                end
                                gate=parity_gate(AA1_temp,4); @tensor AA1_temp[:]:=AA1_temp[-1,-2,-3,1]*gate[-4,1];#sign of U1,U1'
                                if parity_P2==1
                                    gate=parity_gate(AA1_temp,3); @tensor AA1_temp[:]:=AA1_temp[-1,-2,1,-4]*gate[-3,1];#sign P2*(R1+R1')
                ...
                for parity_P3=0:1
                    for parity_P4=0:1
                        for parity_P5=0:1
                                ...
                                if parity_P5==1
                                    AA5_temp=deepcopy(AA5_Phyodd_Veven_Veven[cc1,cc2]);
                                elseif parity_P5==0
                                    AA5_temp=deepcopy(AA5_Phyeven_Veven_Veven[cc1,cc2]);
                                end
                                gate=parity_gate(AA1_temp,4); @tensor AA1_temp[:]:=AA1_temp[-1,-2,-3,1]*gate[-4,1];#sign of U1,U1'
                                if parity_P2==1
                                    gate=parity_gate(AA1_temp,3); @tensor AA1_temp[:]:=AA1_temp[-1,-2,1,-4]*gate[-3,1];#sign P2*(R1+R1')
...
v_init=TensorMap(randn, space(AA2_Phyodd,1)*space(AA2_Phyodd,1)*space(AA2_Phyodd,1)*space(AA2_Phyodd,1)*space(AA2_Phyodd,1),Rep[U₁](0=>1));
...
v_init=TensorMap(randn, space(AA2_Phyodd,3)*space(AA2_Phyodd,3)*space(AA2_Phyodd,3)*space(AA2_Phyodd,3)*space(AA2_Phyodd,3),Rep[U₁](0=>1)');
...
@tensor VL[:]:=VL[-1,1,2,3,4,5]*U_L[1,-2,-3]*U_L[2,-4,-5]*U_L[3,-6,-7]*U_L[4,-8,-9]*U_L[5,-10,-11];#dummy, R1',R1,R2',R2,R3',R3,R4',R4,R5',R5,R6',R6
VL=permute(VL,(1,2,4,6,8,10,3,5,7,9,11,));#dummy, R1',R2',R3',R4', R1,R2,R3,R4


@tensor VR[:]:=VR[1,2,3,4,5,-11]*U_L'[-1,-2,1]*U_L'[-3,-4,2]*U_L'[-5,-6,3]*U_L'[-7,-8,4]*U_L'[-9,-10,5];#L1',L1,L2',L2,L3',L3,L4',L4,dummy
VR=permute(VR,(2,4,6,8,10,1,3,5,7,9,11,));#L1,L2,L3,L4,L1',L2',L3',L4',dummy




@tensor H[:]:=VL[1,-1,-2,-3,-4,-5,2,3,4,5,6]*VR[2,3,4,5,6,-6,-7,-8,-9,-10,1];#R1',R2',R3',R4' ,L1',L2',L3',L4'
...
ev=permute(ev,(1,2,3,4,5,6,));#L1',L2',L3',L4',dummy
...
if y_anti_pbc
    matwrite("ES_freefermion_M2_Nv5_APBC"*".mat", Dict(
        "k_phase" => k_phase,
        "eu" => eu,
        "Qn"=>Qn
    ); compress = false)
else
    matwrite("ES_freefermion_M2_Nv5"*".mat", Dict(
        "k_phase" => k_phase,
        "eu" => eu,
        "Qn"=>Qn
    ); compress = false)
