        ...
        for cc2=1:l_Vodd
            for ll1=1:l_total
                for ll2=1:l_total
                    ...
                    @tensor vr_temp[:]:=vr_temp[1,-2,-3,-4,-5]*PPsL[ll1,ll2][-1,1];
                    @tensor vr_temp[:]:=vr_temp[1,2,3,4,-5]*gate[-1,1]*gate[-2,2]*gate[-3,3]*gate[-4,4];
                    vr=vr+vr_temp;
                end
            end
        end
        ...
        for cc2=1:l_Vodd
            for ll1=1:l_total
                for ll2=1:l_total
                    ...
                    gate=parity_gate(AA1_temp,4); @tensor AA1_temp[:]:=AA1_temp[-1,-2,-3,1]*gate[-4,1];#sign of U1,U1'
                    gate=parity_gate(AA2_temp,1); #sign U1*(L1+L2+L3) + U1'*(L1'+L2'+L3') 
                    @tensor vl_temp[:]:=vl_temp[-1,1,2,3,4]*gate[1,-2]*gate[2,-3]*gate[3,-4]*gate[4,-5];
                    @tensor vl_temp[:]:=AA1_temp[1,2,-2,8]*AA2_temp[3,4,-3,2]*AA3_temp[5,6,-4,4]*AA4_temp[7,8,-5,6]*vl_temp[-1,1,3,5,7];
                    @tensor vl_temp[:]:=vl_temp[-1,1,-3,-4,-5]*PPsR[ll1,ll2][-2,1];
                    vl=vl+vl_temp;
                end
            end
        end
...
v_init=permute(v_init,(1,2,3,4,5,),());#L1,L2,L3,dummy
...
v_init=permute(v_init,(5,1,2,3,4,),());#dummy,R1,R2,R3
...
matwrite("ES_Gutzwiller_M2_decomp_Nv4"*".mat", Dict(
    "k_phase" => k_phase,
    "eu" => eu,
    "Qn"=>Qn,
    "Spin"=>Spin
