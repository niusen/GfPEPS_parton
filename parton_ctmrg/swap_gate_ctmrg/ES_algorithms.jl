function ES_CTMRG_ED_Kprojector(CTM,U_L,U_D,U_R,U_U,M,chi,N,EH_n,group_index)
    ...
    S_Sectors=[0,0,1/2, 1/2, 1, 1, 3/2, 3/2];
    Q_Sectors=[0,2,  1,   3, 2, 4, 3,     5];
    Ks=collect(0:N-1)
    eu_set=Matrix(undef,length(Ks),length(S_Sectors));
    for kk=1:length(Ks)

        for sps=1:length(S_Sectors)
            if N==4
                v_init=TensorMap(randn, space(OO,2)'*space(OO,2)'*space(OO,2)'*space(OO,2)',GradedSpace[Irrep[U₁]⊠Irrep[SU₂]]((Q_Sectors[sps],S_Sectors[sps])=>1));
            ...
            elseif N==6
                v_init=TensorMap(randn, space(OO,2)'*space(OO,2)'*space(OO,2)'*space(OO,2)'*space(OO,2)'*space(OO,2)',GradedSpace[Irrep[U₁]⊠Irrep[SU₂]]((Q_Sectors[sps],S_Sectors[sps])=>1));
            ...
            elseif N==8
                ...
                v_init=TensorMap(randn, fuse(space(OO,2)'*space(OO,2)'*space(OO,2)')*fuse(space(OO,2)'*space(OO,2)'*space(OO,2)')*fuse(space(OO,2)'*space(OO,2)'),GradedSpace[Irrep[U₁]⊠Irrep[SU₂]]((Q_Sectors[sps],S_Sectors[sps])=>1));
            ...
            println("charge: "*string(Q_Sectors[sps]));flush(stdout);
    ...
    ES_filenm="ES_Kprojector"*"_M"*string(M)*"_chi"*string(chi)*"_N"*string(N)*".mat";
...
function ES_CTMRG_ED(CTM,U_L,U_D,U_R,U_U,M,chi,N,EH_n)
    ...
    S_Sectors=[0,0,1/2, 1/2, 1, 1, 3/2, 3/2];
    Q_Sectors=[0,2,  1,   3, 2, 4, 3,     5];
    eu_set=Vector(undef,length(S_Sectors));
    ks_set=Vector(undef,length(S_Sectors));
    # println(fuse(space(OO,2)'*space(OO,2)'*space(OO,2)'*space(OO,2)'))
    for sps=1:length(S_Sectors)
        if N==4
            v_init=TensorMap(randn, space(OO,2)'*space(OO,2)'*space(OO,2)'*space(OO,2)',GradedSpace[Irrep[U₁]⊠Irrep[SU₂]]((Q_Sectors[sps],S_Sectors[sps])=>1));
            v_init=permute(v_init,(1,2,3,4,5,),());
    ...
    ES_filenm="ES_spin"*"_M"*string(M)*"_chi"*string(chi)*"_N"*string(N)*".mat";
    matwrite(ES_filenm, Dict(
        "eu_set" => eu_set,
        "S_Sectors" => S_Sectors,
        "Q_Sectors" => Q_Sectors,
