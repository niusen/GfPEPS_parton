...
for cb=1:N_boundary
    ...
    y_anti_pbc=true;
    boundary_phase_y=cb/N_boundary;
    ...
    AA2=permute_neighbour_ind(AA2,1,2,4);#U2,L2,R2,D2
    @tensor AA1AA2[:]:=AA1[-1,-2,-3,1]*AA2[1,-4,-5,-6];#L1,U1,R1,L2,R2,D2
    AA1AA2=permute_neighbour_ind(AA1AA2,2,3,6);#L1,R1,U1,L2,R2,D2
    AA1AA2=permute_neighbour_ind(AA1AA2,3,4,6);#L1,R1,L2,U1,R2,D2
    AA1AA2=permute_neighbour_ind(AA1AA2,4,5,6);#L1,R1,L2,R2,U1,D2
    AA1AA2=permute_neighbour_ind(AA1AA2,5,6,6);#L1,R1,L2,R2,D2,U1
    @tensor AA1AA2[:]:=AA1AA2[-1,-2,-3,-4,1,1];#L1,R1,L2,R2

    AA1AA2=permute_neighbour_ind(AA1AA2,2,3,4);#L1,L2,R1,R2



    @tensor AA1AA2[:]:=AA1AA2[1,2,3,4]*U_L'[-1,-2,1]*U_L'[-3,-4,2]*U_L[3,-5,-6]*U_L[4,-7,-8];#L1',L1,L2',L2,R1',R1,R2',R2
    AA1AA2=permute(AA1AA2,(1,2,3,4,6,5,8,7,));#L1',L1,L2',L2,R1,R1',R2,R2'



    function M_vr(mpo,vr)
        vr=deepcopy(vr);#L1',L1,L2',L2
        mpo=deepcopy(mpo);#L1',L1,L2',L2,R1,R1',R2,R2'

        vr=permute_neighbour_ind(vr,2,3,5);#L1',L2',L1,L2
        vr=permute_neighbour_ind(vr,1,2,5);#L2',L1',L1,L2
        vr=permute_neighbour_ind(vr,3,4,5);#L2',L1',L2,L1
        vr=permute_neighbour_ind(vr,2,3,5);#L2',L2,L1',L1

        @tensor vr[:]:=mpo[-1,-2,-3,-4,1,2,3,4]*vr[4,3,2,1,-5];
        vr=permute(vr,(1,2,3,4,5,));
        return vr;
    end

    function vl_M(mpo,vl)
        vl=deepcopy(vl);#dummy,R1,R1',R2,R2'
        mpo=deepcopy(mpo);#L1',L1,L2',L2,R1,R1',R2,R2'

        vl=permute_neighbour_ind(vl,3,4,5);#dummy,R1,R2,R1',R2'
        vl=permute_neighbour_ind(vl,4,5,5);#dummy,R1,R2,R2',R1'
        vl=permute_neighbour_ind(vl,2,3,5);#dummy,R2,R1,R2',R1'
        vl=permute_neighbour_ind(vl,3,4,5);#dummy,R2,R2',R1,R1'

        @tensor vl[:]:=vl[-1,4,3,2,1]*mpo[1,2,3,4,-2,-3,-4,-5];
        vl=permute(vl,(1,2,3,4,5,));
        return vl;
    end
    v_init=TensorMap(randn, space(AA1AA2,1)*space(AA1AA2,2)*space(AA1AA2,3)*space(AA1AA2,4),Rep[U₁](0=>1));
    v_init=permute(v_init,(1,2,3,4,5,),());#L1',L1,L2',L2,dummy
    contraction_fun_R(x)=M_vr(AA1AA2,x);
    @time eur,evr=eigsolve(contraction_fun_R, v_init, 1,:LM,Arnoldi(krylovdim=10));
    VR=evr[findmax(abs.(eur))[2]];#L1',L1,L2',L2,dummy

    v_init=TensorMap(randn, space(AA1AA2,5)*space(AA1AA2,6)*space(AA1AA2,7)*space(AA1AA2,8),Rep[U₁](0=>1)');
    v_init=permute(v_init,(5,1,2,3,4,),());#dummy,R1,R1',R2,R2'
    contraction_fun_L(x)=vl_M(AA1AA2,x);
    @time eul,evl=eigsolve(contraction_fun_L, v_init, 1,:LM,Arnoldi(krylovdim=10));
    VL=evl[findmax(abs.(eul))[2]];#dummy,R1,R1',R2,R2'
    ...
    k_phase=imag(log.((diag(k_phase)))/(2*pi));

    Es=vcat(Es,eu);
    dk=1/2;
    Ks=vcat(Ks,k_phase.+boundary_phase_y*dk);
...
Es=-log.(Es);
plot(scatter(x=Ks, y=Es, mode="markers"))