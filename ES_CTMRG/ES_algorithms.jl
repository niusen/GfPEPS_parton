...
function ES_CTMRG_ED_Kprojector(CTM,U_L,U_D,U_R,U_U,M,chi,N,EH_n,decomp=false)
    k_phase=[];
    eu=[];
    Qn=[];
    Spin=[];
    ...
    order=findall(x -> mod(x,1)<3, S_Sectors);
    ...
    if decomp
        ES_filenm="ES_CTMite_decomp_Kprojector_Gutzwiller"*"_M"*string(M)*"_N"*string(N)*"_chi"*string(chi)*".mat";
    else
        ES_filenm="ES_CTMite_Kprojector_Gutzwiller"*"_M"*string(M)*"_N"*string(N)*"_chi"*string(chi)*".mat";
    end
...
function ES_CTMRG_ED(CTM,U_L,U_D,U_R,U_U,M,chi,N,EH_n,decomp=false)
    k_phase=[];
    eu=[];
    Qn=[];
    Spin=[];
    ...
    if decomp
        ES_filenm="ES_CTMite_decomp_Gutzwiller"*"_M"*string(M)*"_N"*string(N)*"_chi"*string(chi)*".mat";
    else
        ES_filenm="ES_CTMite_Gutzwiller"*"_M"*string(M)*"_N"*string(N)*"_chi"*string(chi)*".mat";
    end
    matwrite(ES_filenm, Dict(
        "k_phase" => k_phase,
        "eu" => eu,
