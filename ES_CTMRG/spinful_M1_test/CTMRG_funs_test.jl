...
function CTM_ite(Cset, Tset, AA_fused, chi, direction, trun_tol,CTM_ite_info)
    ...
    if direction==1
        ...
        @tensor PM_[:]:=PM[-1,1,-3]*gate[1,-2];
        @tensor Tleft_gate[:]:=Tleft_gate[4,3,1]*AA[3,5,-2,2]* PM_inv[4,5,-1]* PM_[1,2,-3];
        Tleft_gate=Tleft_gate/norm(Tleft_gate);
    elseif direction==3
        gate=parity_gate(AA,2);
        @tensor PM_inv_[:]:=PM_inv[-1,1,-3]*gate[1,-2];
        @tensor Tright_gate[:]:=Tright_gate[4,3,1]*AA[3,5,-2,2]* PM_inv_[4,5,-1]* PM[1,2,-3];
        Tright_gate=Tright_gate/norm(Tright_gate);
    end
